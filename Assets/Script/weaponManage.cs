	...
	void Update() {
		...
		if (myPV.isMine) {
			...
			if (reloadTimer > wep01rate && variableManage.fireWeapon && wep01currentNum > 0) {
				...
				myPV.RPC("fireBullet",
						PhotonTargets.All,
						wep01spd, wep01name, wep01atkPow,
			...
			if (lockCalcTimer > 0.5f) {
				...
				foreach (GameObject obj in allEnemy) {
					...
					if (tmpDist < 180f && obj != this.gameObject) {
						//float tmpDist = Vector3.Distance(obj.transform.position, this.transform.position);
						if (tmpDist < bestDist) {
			...
			if (variableManage.lockoned) {
				...
				Vector2 myYpos = new Vector2(fireMouse.transform.position.x, fireMouse.transform.position.z);
				...
				Vector2 myXpos = new Vector2(xRotObject.transform.position.y, xRotObject.transform.position.z);
				...
				yRotObject.transform.localRotation =
					Quaternion.Slerp(yRotObject.transform.localRotation, Quaternion.Euler(new Vector3(0f, yAngl, 0f)), 0.01f);
				xRotObject.transform.localRotation =
					Quaternion.Slerp(xRotObject.transform.localRotation, Quaternion.Euler(new Vector3(xAngl, 0f, 0f)), 0.01f);
			} else {
				//非ロックオン中
				yRotObject.transform.localRotation =
					Quaternion.Slerp(yRotObject.transform.localRotation, Quaternion.Euler(new Vector3(0f, 0f, 0f)), 0.01f);
				xRotObject.transform.localRotation =
					Quaternion.Slerp(xRotObject.transform.localRotation, Quaternion.Euler(new Vector3(0f, 0f, 0f)), 0.01f);
			}
		}
	}
